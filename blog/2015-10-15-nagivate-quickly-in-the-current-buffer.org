...
注：关于插件和 Emacs 的用法，最好的方法其实是录制视频或者制作 Gif，但是录制视频真的花时间，因为还要上传。这里先欠着，后面的[[https://github.com/zilongshanren/Spacemacs-rocks][zilongshanren/Spacemacs-rocks]] 再补上。
...
如果要搜索当前光标下面的符号，我会使用[[https://github.com/bling/evil-visualstar][bling/evil-visualstar]] 来搜索，只需要把光标停在你想要搜索的符号上面，然后按一个*就可以了。

通过[[https://github.com/syohex/emacs-evil-anzu][syohex/emacs-evil-anzu]] 可以在状态栏显示出当前匹配的 item 个数。

它最大的优点就是简单方便，native to vim 用户。

** Swiper

此插件是我最喜爱的 Lisp 黑客 [[https://github.com/abo-abo][abo-abo (Oleh Krehel)]] 所写，我的 Emacs 里面配置了很多他写的插件，最好用的是 lispy，hydra，avy 和 ace-window。
大家从他的 Github 主页上面也可以看出来这些插件在 Emacs 社区的受欢迎程度。
...
如果在 emacs state 我一般会选择使用 swiper 来插件想要的内容，而不会先回到 evil normal 状态，再按/来搜索。

关于 Swiper 更多的介绍可以参考原作者的项目文档和博客。

它最大的优点就是可以在不切换模式的前提下提供最方便地搜索和预览功能。
...
我在 reddit 里面提问就解决了一个 mocha 测试文件里面查找测试例的问题。[[https://www.reddit.com/r/emacs/comments/3ol0b6/how_to_show_mocha_test_names_in_imenu_list/][How to show mocha test names in imenu list. : emacs]] 
